...
struct RunResult {
    ...
    // Mean acceptance over chains, measured after burn-in
    d64 acceptedFraction = std::numeric_limits<d64>::quiet_NaN();
 
    // Pooled across chains, one entry per observable. tauInt, ess and
    // stdError come from the split multi-chain estimator
    std::vector<d64> mean;
    std::vector<d64> stdError;
    std::vector<d64> tauInt; // in draws
...
RunResult run(Factory&& makeStepper, const Obs& observables, const RunConfig& cfg, Harvest&& harvest = {}) {
    ...
    if (cfg.sweeps % cfg.thin != 0) {
        throw std::invalid_argument("run: sweeps must be a whole multiple of thin");
    }
    ...
    d64 accSum = 0.0;
    u32 accCount = 0;
    for (const ChainSummary& cs : res.chains) {
        if (!std::isnan(cs.acceptedFraction)) {
            accSum += cs.acceptedFraction;
            ++accCount;
        }
    }
    if (accCount) {
        res.acceptedFraction = accSum / static_cast<d64>(accCount);
    }
    ...
    for (u32 o = 0; o < res.numObs; ++o) {
        ...
        const stats::EssResult combined = stats::splitEss(spans, maxLag);
 
        res.mean[o] = pooledMean;
        res.tauInt[o] = combined.tau;
        res.tauSweeps[o] = combined.tau * static_cast<d64>(res.thin);
        res.ess[o] = combined.ess;
        res.stdError[o] = combined.ess > 0.0
            ? std::sqrt(combined.varPlus / combined.ess)
...
inline void save(const RunResult& res, const std::string& stem, const RunConfig& cfg, const io::JsonWriter& extra = io::JsonWriter{}) {
    ...
    j.add("accepted_fraction", acc)
        .add("accepted_fraction_mean", res.acceptedFraction);
